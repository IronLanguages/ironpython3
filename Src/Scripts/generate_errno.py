...
def generate_errno_names(cw):
    ...
    common_names = sorted(
        name for name in errorvalues
            if all(errorvalues[name]) and ("WSA" + name not in errorvalues or errorvalues["WSA" + name][windows_idx] != errorvalues[name][windows_idx]) and name not in linux_aliases.keys())
    ...
    posix_names = sorted(
        name for name in errorvalues
            if errorvalues[name][linux_idx] and errorvalues[name][macosx_idx] and name not in common_names + list(linux_aliases.keys()))
    ...
    linux_names = sorted(
        name for name in errorvalues
            if errorvalues[name][linux_idx] and name not in common_names + posix_names + list(linux_aliases.keys()))
    ...
    macosx_names = sorted(
        name for name in errorvalues
            if errorvalues[name][macosx_idx] and name not in common_names + posix_names)
