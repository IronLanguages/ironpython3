...
class SocketTest(IronPythonTestCase):
    ...
    @retryOnFailure
    def test_cp12452(self):
        '''Fully test _socket._fileobj compatibility'''
        expected_dir = [
                        '__module__',
        ...
        fileobject_dir = dir(_socket._fileobject)
    ...
    @retryOnFailure
    def test_fileobject_close(self):
        """verify we can construct fileobjects w/ the close kw arg"""
        fd = _socket._fileobject(None, close=True)
        self.assertEqual(fd.mode, 'rb')
        self.assertEqual(fd.closed, True)
