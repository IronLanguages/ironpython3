...
def gen_one_new_exception(cw, exception, parent):
    if exception.fields:
        ...
        cw.writeline('%sStorage = CreateSubType(%s, typeof(_%s), (msg, innerException) => new %s(msg, innerException));' % (exception.name, exception.parent.PythonType, exception.name, exception.DotNetExceptionName))
        cw.exit_block() # lock
        cw.exit_block() # if
        ...
        cw.enter_block('if (%sStorage == null)' % (exception.name, ))
        cw.enter_block('lock (_pythonExceptionsLock)')
        cw.writeline('%sStorage = CreateSubType(%s, "%s", (msg, innerException) => new %s(msg, innerException));' % (exception.name, exception.parent.PythonType, exception.name, exception.DotNetExceptionName))
        cw.exit_block() # lock
        cw.exit_block() # if
        cw.writeline('return %sStorage;' % (exception.name, ))
        cw.exit_block()
