...
class InstanceFieldsTest(IronPythonTestCase):
    ...
    def _test_set_by_instance(self, o, vf, t):
        ...
        for i in range(1, 25):
            ...
            if clr.GetClrType(t).IsValueType:
                self.assertWarnsRegex(RuntimeWarning, 'may result in updating a copy', f)
            else:
                f()

        if not clr.GetClrType(t).IsValueType:
            self._test_verify(v)
    ...
    def _test_set_by_descriptor(self, o, vf, t):
        ...
        for i in range(1, 25):
            f = eval("f%s" % i)
            if clr.GetClrType(t).IsValueType:
                self.assertWarns(RuntimeWarning, f)
            else:
                f()
    ...
    def _test_delete_by_type(self, current_type):
        ...
        for i in range(1, 25):
            self.assertRaisesRegex(AttributeError, "cannot delete attribute", eval("f%s" % i))
    ...
    def test_generic_fields(self):
        ...
        self.assertWarnsRegex(RuntimeWarning, 'may result in updating a copy', tryUpdate)
