...
def set_value(errorname, name, value, index):
    if name not in errorname:
        errorname[name] = [None] * num_systems
    errorname[name][index] = value
...
def generate_errno_codes(cw):
    ...
    for code in errorcode_linux.keys():
        set_value(errorname, errorcode_linux[code], code,  linux_idx)
    for code in errorcode_macosx.keys():
        set_value(errorname, errorcode_macosx[code], code, macosx_idx)
    for code in errorcode_windows.keys():
        set_value(errorname, errorcode_windows[code], code, windows_idx)

    names = list(sorted(errorname, key = lambda x: 
        errorname[x][linux_idx] or (errorname[x][macosx_idx] + 0.1 if errorname[x][macosx_idx] else None) or errorname[x][windows_idx] + 0.2))
    for name in names:
        codes = errorname[name]
        hidden_on = []
        cw.writeline()
